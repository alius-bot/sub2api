...
func (s *GatewayService) ForwardKiro(ctx context.Context, c *gin.Context, account *Account, parsed *ParsedRequest, startTime time.Time) (*ForwardResult, error) {
	...
	out := anthropicResponse{
		...
		Role:       "assistant",
		Model:      originalModel,
		StopReason: kiroDefaultString(kiroResp.StopReason, "end_turn"),
...
type kiroResponseCaptureWriter struct {
	gin.ResponseWriter
	header http.Header
...
func (w *kiroResponseCaptureWriter) Header() http.Header {
	return w.header
}

func (w *kiroResponseCaptureWriter) WriteHeader(code int) {
	w.status = code
}

func (w *kiroResponseCaptureWriter) Write(data []byte) (int, error) {
	return w.body.Write(data)
}

func (w *kiroResponseCaptureWriter) WriteString(s string) (int, error) {
	return w.body.WriteString(s)
}

// anthropicResponse and anthropicContentBlock are local aliases for JSON marshalling.
...
func kiroCredentialsFromAccount(account *Account) kiro.Credentials {
	return kiro.Credentials{
		...
		AccessToken:           kiroFirstCredential(account, "access_token", "accessToken"),
		RefreshToken:          kiroFirstCredential(account, "refresh_token", "refreshToken"),
		ClientID:              kiroFirstCredential(account, "client_id", "clientId"),
		ClientSecret:          kiroFirstCredential(account, "client_secret", "clientSecret"),
		ClientSecretExpiresAt: account.GetCredentialAsInt64("client_secret_expires_at"),
		AuthMethod:            kiroFirstCredential(account, "auth_method", "authMethod"),
		ExpiresAt:             kiroFirstCredential(account, "expires_at", "expiresAt"),
		ProfileARN:            kiroFirstCredential(account, "profile_arn", "profileArn"),
		Region:                kiroDefaultString(account.GetCredential("region"), kiro.DefaultRegion),
	}
}
...
func kiroFirstCredential(account *Account, keys ...string) string {
...
func kiroDefaultString(v, fallback string) string {
