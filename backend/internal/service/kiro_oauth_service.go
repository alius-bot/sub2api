...
// Stop shuts down the social callback HTTP server if running.
func (s *KiroOAuthService) Stop() {
	...
	if s.callbackServer != nil {
		...
		defer cancel()
		_ = s.callbackServer.Shutdown(ctx)
...
func (s *KiroOAuthService) StartDeviceAuth(ctx context.Context, input KiroDeviceAuthInput) (*KiroDeviceAuthResult, error) {
	...
	return &KiroDeviceAuthResult{
		...
		AuthURL:         kiroFirstNonEmpty(session.VerificationURIComplete, session.VerificationURI),
...
func (s *KiroOAuthService) StartSocialAuth(ctx context.Context, input KiroSocialAuthInput) (*KiroDeviceAuthResult, error) {
	...
	s.sessionStore.Set(session)
	if kiroIsLoopbackCallback(session.RedirectURI) {
...
func (s *KiroOAuthService) handleSocialCallback(w http.ResponseWriter, r *http.Request) {
	...
	creds, status, err := kiro.CompleteSocialAuthorization(r.Context(), session, kiroCallbackURLForRequest(r))
...
func (s *KiroOAuthService) BuildAccountCredentials(creds kiro.Credentials) map[string]any {
	out := map[string]any{
		...
		"region":        kiroFirstNonEmpty(creds.Region, kiro.DefaultRegion),
	}
...
func (s *KiroOAuthService) ensureSocialCallbackServer(redirectURI string) error {
	addr, err := kiroCallbackListenAddr(redirectURI)
	if err != nil {
		return err
	}
...
func kiroCallbackURLForRequest(r *http.Request) string {
	scheme := "http"
	if r.TLS != nil {
		scheme = "https"
	}
...
func kiroCallbackListenAddr(raw string) (string, error) {
	host, port, err := kiroCallbackHostPort(raw)
	if err != nil {
		return "", err
	}
...
func kiroCallbackHostPort(raw string) (string, string, error) {
...
func kiroIsLoopbackCallback(raw string) bool {
	host, _, err := kiroCallbackHostPort(raw)
	if err != nil {
		return false
	}
	ip := net.ParseIP(host)
...
func kiroFirstNonEmpty(vals ...string) string {
	for _, v := range vals {
